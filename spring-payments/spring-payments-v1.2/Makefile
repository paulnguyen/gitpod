...
run-jar: build
	java -jar target/spring-payments-1.2.jar
...
docker-push:
	docker login
	docker build -t $(account)/spring-payments:v1.2 -t $(account)/spring-payments:v1.2 .
	docker push $(account)/spring-payments:v1.2
...
network-create:
	docker network create --driver bridge spring-payments-v1.2_network
